...
export default function Dashboard() {
  ...
  return (
    ...
    >
      ...
      {/* 1. Page Title Group */}
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-slate-100 pb-8 md:pb-10 relative px-4 md:px-0">
        <div className="absolute -left-4 md:-left-10 top-0 w-1 h-12 bg-[#0052a5] rounded-full blur-[2px]"></div>
        <div className="space-y-1">
          <p className="text-[9px] md:text-[10px] uppercase font-black tracking-[0.25em] text-[#0052a5] mb-2 flex items-center gap-2">
             <div className="w-1.5 h-1.5 rounded-full bg-blue-500"></div>
             Node Status: Active
          </p>
          <h2 className="text-2xl md:text-4xl font-black text-slate-900 tracking-tighter leading-none mb-2 uppercaseLeading-none">Resource Command</h2>
          <p className="text-[11px] md:text-[13px] font-bold text-slate-400 uppercase tracking-widest leading-none">Matrix Operational Oversight</p>
        </div>
        
        <div className="flex items-center gap-4 bg-white/50 backdrop-blur-xl px-5 py-3 md:px-6 md:py-4 rounded-2xl md:rounded-[32px] border border-white shadow-sm ring-1 ring-slate-100/50 w-full md:w-auto">
           <div className="w-10 h-10 rounded-xl md:rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600 shadow-sm border border-emerald-100">
              <Zap size={20} md-size={22} fill="currentColor" strokeWidth={3} />
           </div>
           <div className="cursor-pointer group flex-1" onClick={() => router.push('/labs')}>
              <p className="text-[9px] md:text-[10px] uppercase font-black text-slate-400 tracking-wider leading-none mb-1 group-hover:text-[#0052a5] transition-colors">Matrix Health</p>
              <p className="text-xs md:text-sm font-black text-slate-900 tracking-tight leading-none group-hover:underline decoration-[#0052a5] underline-offset-4">Encrypted / Stable</p>
           </div>
        </div>
      </div>
      ...
      <motion.div 
        ...
        className="bg-[#0052a5] rounded-3xl md:rounded-[48px] p-6 sm:p-8 md:p-12 text-white shadow-3xl shadow-blue-900/40 relative overflow-hidden flex flex-col justify-between min-h-[360px] md:min-h-[440px] group transition-all duration-700 hover:shadow-blue-900/60"
      >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start mb-12 md:mb-16 relative z-10 gap-8">
          <div>
            <div className="px-5 py-2 md:px-6 md:py-2 bg-white/10 text-white text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] rounded-full backdrop-blur-md border border-white/20 inline-flex items-center gap-2 mb-6 md:mb-10 shadow-lg">
               <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-emerald-500 animate-pulse"></div>
               Global Roster Active
            </div>
            <h3 className="text-3xl md:text-6xl font-black mb-4 tracking-tighter leading-[0.95] md:leading-[0.9] max-w-xl group-hover:translate-x-2 transition-transform duration-500">Institutional <br className="hidden md:block" /> Intelligence Matrix</h3>
            <p className="text-blue-200 font-bold uppercase tracking-[0.2em] text-[11px] md:text-[13px] opacity-80 pl-1">Synchronizing {stats?.totalLabs} Active Nodes</p>
          </div>
          <button 
             onClick={() => router.push('/labs')}
             className="w-full md:w-auto bg-white text-[#0052a5] px-8 py-4 md:px-10 md:py-6 rounded-2xl md:rounded-3xl font-black text-[11px] md:text-[12px] uppercase tracking-[0.2em] md:tracking-[0.25em] shadow-2xl shadow-blue-900/20 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-4 group"
          >
             Initialize Lab <ArrowUpRight size={20} md-size={22} strokeWidth={3} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
          </button>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-12 pt-10 md:pt-12 border-t border-white/15 relative z-10">
          {[
            { label: "Matrix Labs", value: stats?.totalLabs, sub: "Verified", path: "/labs" },
            { label: "Personnel", value: stats?.totalStudents, sub: "Identity", path: "/attendance" },
            { label: "Nodes", value: stats?.totalLabs ? stats.totalLabs * 12 : 0, sub: "Telemetry", path: "/inventory" },
            { label: "Uptime", value: "99.9%", sub: "SLA", path: "/reports" }
          ].map((stat, i) => (
             <div 
               key={i} 
             ...
             >
               <p className="text-[9px] md:text-[11px] text-blue-200/60 uppercase font-black tracking-[0.2em] mb-2 md:mb-4 group-hover/stat:text-white transition-colors">{stat.label}</p>
               <p className="text-2xl md:text-5xl font-black tracking-tight leading-none mb-1 md:mb-3 group-hover/stat:scale-110 transition-transform origin-left">{stat.value}</p>
               <p className="text-[8px] md:text-[10px] font-bold text-blue-200/40 uppercase tracking-widest">{stat.sub}</p>
             </div>
          ))}
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-12 gap-10">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-8 md:space-y-10">
           <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-4 md:mb-6 px-2 md:px-0">
              <div>
                <p className="text-[9px] md:text-[10px] uppercase font-black tracking-[0.3em] text-[#0052a5] mb-2">Live Registry</p>
                <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter">Recent Handshakes</h3>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                 <div className="flex bg-slate-100 p-1 rounded-xl md:rounded-2xl border border-slate-200 shadow-inner">
                    {["All", "IT", "ECE"].map(cat => (
                       <button 
                         ...
                         className={`px-4 py-2 rounded-lg md:rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${filterCategory === cat ? 'bg-white text-[#0052a5] shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                       >
                          {cat}
                       </button>
                    ))}
                 ...
                 <button 
                    onClick={() => router.push('/labs')}
                    className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-white border border-slate-100 hover:border-[#0052a5] text-slate-600 font-black text-[9px] md:text-[10px] uppercase tracking-widest rounded-xl md:rounded-2xl transition-all shadow-sm group"
                 >
                    <Filter size={14} md-size={16} /> All Labs
                 </button>
              </div>
           </div>
              ...
              <AnimatePresence>
                 {(filteredLabs.length > 0 ? filteredLabs : stats?.recentLabs || []).map((lab) => (
                    <motion.div 
                      ...
                      variants={itemVariants}
                      whileHover={{ y: -5 }}
                      className="bg-white rounded-3xl md:rounded-[40px] p-6 md:p-10 shadow-sm border border-slate-100 hover:shadow-3xl hover:shadow-blue-900/5 transition-all group relative overflow-hidden cursor-pointer"
                    ...
                    >
                       ...
                       <div className="flex items-center justify-between mb-8 md:mb-10 relative z-10">
                          <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl md:rounded-[24px] bg-blue-50 flex items-center justify-center text-[#0052a5] group-hover:bg-[#0052a5] group-hover:text-white group-hover:scale-110 transition-all duration-500 shadow-lg shadow-blue-900/5">
                             <Microscope size={28} md-size={32} strokeWidth={2.5} />
                          </div>
                          <div className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-slate-100 flex items-center justify-center text-slate-300 group-hover:text-[#0052a5] group-hover:border-blue-100 group-hover:bg-blue-50 transition-all shadow-sm">
                             <ExternalLink size={18} md-size={20} />
                          </div>
                       </div>
                       <h4 className="text-xl md:text-2xl font-black text-slate-900 mb-2 tracking-tight group-hover:text-[#0052a5] transition-colors">{lab.name}</h4>
                       <p className="text-[10px] md:text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6 md:mb-8">{lab.department} / Secure Node</p>
                       <div className="flex items-center gap-4 py-3 md:py-4 px-5 md:px-6 bg-slate-50/50 rounded-2xl md:rounded-3xl border border-slate-100 group-hover:bg-blue-50 transition-colors">
                          <div className="w-2 h-2 md:w-2.5 md:h-2.5 bg-emerald-500 rounded-full shadow-[0_0_8px_#10b981]"></div>
                          <span className="text-[9px] md:text-[10px] font-black text-slate-600 uppercase tracking-widest">Active Roster Online</span>
                       </div>
                    </motion.div>
                 ))}
              </AnimatePresence>
           ...
           <div className="pt-8 md:pt-10 space-y-8 md:space-y-10 px-2 md:px-0">
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-[9px] md:text-[10px] uppercase font-black tracking-[0.3em] text-[#0052a5] mb-2">Infrastructure</p>
                  <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter">Diagnostic Nodes</h3>
                </div>
              </div>
